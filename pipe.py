...
class Pipe:
    def __init__(self,scale_factor,move_speed):
        self.img_up=pg.transform.scale_by(pg.image.load("Assests/pipe.png").convert_alpha(),scale_factor)
        self.img_down=pg.transform.scale_by(pg.image.load("Assests/pipe2.png").convert_alpha(),scale_factor)
        self.rect_up=self.img_up.get_rect()
        self.rect_down=self.img_down.get_rect()
        self.pipe_distance=200
        self.rect_up.y=randint(240,500)
        self.rect_up.x=600
